LDFLAGS=-I headers -I headers/token
...
input_file.o:
	g++ -Wall $(LDFLAGS) sources/input_file.cpp -c

input_stdin_type.o:
	g++ -Wall $(LDFLAGS) sources/input_stdin_type.cpp -c

analysing.o: analysing.cpp
	g++ -Wall $(LDFLAGS) analysing.cpp -c

counting.o:
	g++ -Wall $(LDFLAGS) sources/token/counting.cpp -c

token.o:
	g++ -Wall $(LDFLAGS) sources/token/token.cpp -c

main_gcov.o:
	g++ -Wall $(LDFLAGS)  main.cpp -coverage -fprofile-arcs -ftest-coverage -lgcov -c

input_file_gcov.o:
	g++ -Wall $(LDFLAGS) sources/input_file.cpp -coverage -fprofile-arcs -ftest-coverage -lgcov -c

input_stdin_type_gcov.o:
	g++ -Wall $(LDFLAGS) sources/input_stdin_type.cpp -coverage -fprofile-arcs -ftest-coverage -lgcov -c
...
counting_gcov.o:
	g++ -Wall $(LDFLAGS) sources/token/counting.cpp -coverage -fprofile-arcs -ftest-coverage -lgcov -c

token_gcov.o:
	g++ -Wall $(LDFLAGS) sources/token/token.cpp  -coverage -fprofile-arcs -ftest-coverage -lgcov -c

clean:
	rm *.gcda *.gcno *.o main.out *.info